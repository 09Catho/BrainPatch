...
def compile_reference_patch(
    ...
    compatibility = {
        ...
        "llamacpp": {
            "status": "verified",
            "llamacpp_build": "b10344 (7a20b417f)",
            "gguf_quantizations": ["Q4_K_M"],
            "gguf": "Qwen/Qwen2.5-1.5B-Instruct-GGUF/qwen2.5-1.5b-instruct-q4_k_m.gguf",
            "verified_by": "modal run modal_app/llamacpp_verify.py::verify_llamacpp",
            "checks": [
                "layer_mapping_0based_to_1based_direction_tensors",
                "scale_zero_matches_baseline",
                "nonzero_scale_changes_output",
                "layer_range_honoured",
                "no_crash",
            ],
            "capability_gap": "no token schedules: a control vector is bound for a whole run",
        },
        "vllm": {
            "status": "verified",
            "vllm_version": "0.11.0",
            "mechanism": "worker_extension_cls + collective_rpc (no insecure serialization)",
            "verified_by": "modal run modal_app/vllm_verify.py::verify_vllm and ::verify_vllm_server",
            "checks": [
                "hooks_installed_inside_vllm_worker_process",
                "scale_zero_matches_baseline",
                "nonzero_scale_changes_output",
                "batched_requests_agree_with_single",
                "openai_server_concurrent_requests_no_state_leak",
                "mismatched_per_request_strength_rejected_400",
            ],
            "capability_gap": "no per-request strength: continuous batching shares one forward pass",
        },
        "mlx-lm": {
            "status": "experimental",
